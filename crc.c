...
void xor () {
  for (c = 1; c < N; c++)
    //
    cs[c] = ((cs[c] == g[c]) ? '0' : '1');
  // Checking the XOR operation. If both operands are same, then output will b
  // "0" otherwise its "1".
} void crc() {
  for (e = 0; e < N; e++)
  ...
  do {
    ...
    for (c = 0; c < N - 1; c++)
      // Performing XOR operation at the first iteration for FIVE bits (0 to
      // N-1)
      cs[c] = cs[c + 1];
...
int main() {
  ...
  for (e = a; e < a + N - 1; e++)
    // To append the checksum value instead of N-1 zeros in total length of the
    // data
    t[e] = cs[e - a];
